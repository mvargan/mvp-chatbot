...
app.post("/chat", (req, res) => {
  ...
  const out = formatAnswer(hits);
  if (!out) return res.json({ reply: outOfScopeReply(), sources: [] });
...
function buildIndex(chunks) {
  return lunr(function () {
    ...
    this.field("title");
    this.field("text");
    this.pipeline.remove(lunr.stemmer); // ok for Q&A docs
...
function smallTalkReply(intent) {
  switch (intent) {
    ...
    case "thanks":
      return "You're welcome! If you tell me the country (HR/PL/IT/GR/LT) and the topic, I can answer faster.";
    case "howareyou":
      return "I'm great — ready to help with the S.O.S. project. What topic are you working on?";
    case "identity":
      return "I'm the S.O.S. chatbot for a school Erasmus project. I answer only from the S.O.S. knowledge base.";
    case "help":
      return "Try: choose a country (HR/PL/IT/GR/LT) + topic (science/engineering/art/green jobs). Example: \"Green jobs in Italy related to art?\"";
    default:
      return null;
  }
}
...
function inappropriateReply() {
  return "I can't help with that. I'm a school project chatbot focused on the S.O.S. Erasmus topics. Please ask a respectful question about the project.";
}
...
function pickTop(query, k = 4) {
  if (!KB_INDEX) return [];
  const results = KB_INDEX.search(query).slice(0, k);
  const byId = new Map(KB_CHUNKS.map((c) => [c.id, c]));
  return results.map((r) => ({ score: r.score, chunk: byId.get(r.ref) })).filter((x) => x.chunk);
}
...
function formatAnswer(hits) {
  ...
  const snippets = hits.slice(0, 3).map((h, i) => {
  ...
  return {
    reply:
      "Here's what the S.O.S. knowledge base says:\n\n" +
      snippets +
      "\n\nIf you want, tell me the country + topic and I'll narrow it down.",
    sources: hits.slice(0, 3).map((h) => h.chunk.title),
  };
}
